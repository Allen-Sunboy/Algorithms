...
void cut_seam() {
    ...
    if (channel == 3) {
        Sobel(imgNew, gradX, CV_32F, 1, 0);
        Sobel(imgNew, gradY, CV_32F, 0, 1);
    }
    // channel == 1
    else {
        Sobel(imgNew, gradX, CV_16F, 1, 0);
        Sobel(imgNew, gradY, CV_16F, 0, 1);
    }
    ...
    for (int i = 1; i < row; i++) {
        for (int j = 0; j < col; j++) {
            if (j == 0) {
                ...
                s[i][j] =
                    min(s[i - 1][j - 1], min(s[i - 1][j], s[i - 1][j + 1])) +
                    (int)grad.at<uchar>(i, j);
                int p = j - 1;
                if (s[i - 1][p] > s[i - 1][j])
                    p = j;
